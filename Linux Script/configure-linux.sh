...
checkIfLogglyServersAccessible()
{
    ...
    if [ $(curl -k --connect-timeout 10 $LOGS_01_HOST:$LOGGLY_SYSLOG_PORT 2>&1 | grep "Empty reply from server" | wc -l) == 1 ]; then
    ...
    if [ $(curl -k -s --head  --request GET $LOGGLY_ACCOUNT_URL/login | grep "200 OK" | wc -l) == 1 ]; then
    ...
    if [ $(curl -k -s --head  --request GET $LOGGLY_ACCOUNT_URL/apiv2/customer | grep "404 NOT FOUND" | wc -l) == 1 ]; then
...
checkIfValidUserNamePassword()
{
    ...
    if [ $(curl -k -s -u $LOGGLY_USERNAME:$LOGGLY_PASSWORD $LOGGLY_ACCOUNT_URL/apiv2/customer | grep "Unauthorized" | wc -l) == 1 ]; then
...
getAuthToken()
{
    if [ "$LOGGLY_AUTH_TOKEN" = "" ]; then
        ...
        tokenstr=$(curl -k -s -u $LOGGLY_USERNAME:$LOGGLY_PASSWORD $LOGGLY_ACCOUNT_URL/apiv2/customer | grep -v "token")
...
checkIfValidAuthToken()
{
    ...
    if [ $(curl -k -s -u $LOGGLY_USERNAME:$LOGGLY_PASSWORD $LOGGLY_ACCOUNT_URL/apiv2/customer | grep \"$LOGGLY_AUTH_TOKEN\" | wc -l) == 1 ]; then
...
sendPayloadToConfigSysLog()
{
    ...
    curl -k -s -H "content-type:application/json" -d "$var" $LOGS_01_URL/inputs/$3 > /dev/null 2>&1
}
...
searchAndFetch()
{
    ...
    result=$(wget --no-check-certificate -qO- /dev/null --user "$LOGGLY_USERNAME" --password "$LOGGLY_PASSWORD" "$url")
